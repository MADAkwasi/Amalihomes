...
export class FlashSalesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      else if (sort) this.displayProducts.set(sortProducts(this.flashSales(), sort[0] as SortOption));
      else this.displayProducts.set(applyFilters(this.flashSales(), filters));
    });
